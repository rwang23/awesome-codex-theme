const translations = {
  en: {
    ...
    heroKicker: "OPEN STANDARD · ORIGINAL COLLECTION 01",
    heroLineOne: "Codex,",
    ...
    browseThemes: "Browse 8 original themes",
    statThemes: "original themes",
    statCode: "executable files per pack",
    statAdapters: "adapter targets",
    statContrast: "contrast gate",
    collectionEyebrow: "COLLECTION 01 / ORIGINAL XIANXIA",
    collectionTitle: "One world, two expressions.",
    collectionIntro: "Four original worlds, each in cinematic and chibi form. Inspired by the energy of modern Chinese fantasy—not by any existing character or franchise.",
  ...
  "zh-CN": {
    ...
    navStandard: "标准",
    navAdapters: "适配器",
    heroKicker: "开放标准 · 原创系列 01",
    heroLineOne: "Codex，",
    heroLineTwo: "自成一境。",
    heroIntro: "探索不含执行代码的主题包：版权声明清楚、素材可复现、哈希可验证，兼容能力也不夸大。",
    browseThemes: "浏览 8 套原创主题",
    statThemes: "套原创主题",
    statCode: "个主题包可执行文件",
    statAdapters: "个适配目标",
    statContrast: "对比度门槛",
    collectionEyebrow: "系列 01 / 原创国风修仙",
    collectionTitle: "一方世界，两种表达。",
    collectionIntro: "四个原创世界，每个都有原画感与 Q 版两个版本。借的是当代国风幻想的气韵，不借任何既有角色、造型或 IP。",
    searchLabel: "搜索主题",
...
function createCard(theme, index) {
  ...
  number.textContent = String(index + 1).padStart(2, "0") + " / 08";
  ...
  pair.textContent = t("pair") + " " + String(Math.floor(index / 2) + 1).padStart(2, "0");
  meta.append(original, variant, pair);
...
function updateDialog() {
  ...
  elements.dialogVariant.textContent = t(theme.variant) + " · " + t("original");
