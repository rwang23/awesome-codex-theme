...
description: 为 Awesome Codex Theme 创建、补全或校验原创主题。适用于把一句视觉想法变成双语 theme brief、用 OpenAI image job 生成安全留白的源图、写入 themes/catalog.json 与 source-art/jobs.json、导出明暗模式并运行 ACT Registry 校验。Use when a user asks to create, adapt, package, validate, or contribute a Codex theme in this repository.
---

# 创建 Codex 主题

把一个视觉想法交付成可审核的 ACT 主题包。结果必须包含原创源图、中文和英文名称、明暗两套 token、来源记录，以及通过校验的声明式包。

## 先判断任务类型

- 新主题：从 brief 开始，完成源图、catalog、生成与验证。
- 改现有主题：先读原 theme、源图 job 和 provenance，只改用户点名的部分。
- 外部图片适配：先确认用户有再分发权，再决定能否进入 Registry。
- 仅做概念稿：可以停在 brief 和预览图，不要把未审查素材写进 catalog。

如果请求涉及现成动漫、游戏、艺人或具体画师风格，不要复制角色、服装、标志、构图或可识别画风。保留用户想要的题材、节奏和情绪，改成原创世界与原创角色。详细规则见 [references/theme-authoring.md](references/theme-authoring.md)。
...
复制 [assets/theme-brief.template.json](assets/theme-brief.template.json)，填写：
