...
把一个视觉想法交付成可审核、可实际应用的 ACT Full Skin 主题包。结果必须包含已审查源图、中文和英文名称、明暗两套 token、权利轨道、来源记录、通过校验的声明式包，以及固定 Beta 测试台中的真实效果证据。
...
生成器会把一张审查过的源图稳定导出为 2560×1440 明暗背景、960×540 Gallery 预览、`.act-theme` 包、两份 `act-full-skin-v1` Registry 记录和两份 `codex-theme-v1` 原生回退字符串。Full Skin 背景由 Theme Manager 在通过哈希校验后应用；包内仍然不带脚本或 CSS。
...
- 明暗按钮确实切换对应的真实 Beta 截图
- 使用面板明确把 Full Skin 管理器列为主入口，把 `codex-theme-v1` 标成配色回退
- Full Skin 素材、原生回退下载和 `.act-theme` 下载都指向当前主题
- 中文是默认语言，英文切换后没有缺失
- 360px 和桌面宽度都没有横向溢出

构建成功不能替代页面检查。

如果要声明“Codex 实机验证”或提交实机截图，还要按 `docs/native-testing.md` 在对应版本的隔离实例中应用 Full Skin，读回运行时标记并恢复。不要把源图或生成预览写成实机截图。
...
- 人工视觉审查通过
- 浏览器交互通过
- 当前主题的明暗模式都有与准确 Beta 版本绑定的 Full Skin capture，且清理读回通过
