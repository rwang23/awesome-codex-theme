...
生成器会把一张审查过的源图稳定导出为 2560×1440 明暗封面、960×540 Gallery 预览、`.act-theme` 包和两份 `codex-theme-v1` 原生主题字符串。插画是馆藏封面，不会成为 Codex 应用背景。
...
- 明暗按钮确实切换两张封面
- Native 弹窗显示正确主题、模式和 `codex-theme-v1` 字符串
- 复制、原生主题下载和 `.act-theme` 下载都指向当前主题
- 中文是默认语言，英文切换后没有缺失
- 360px 和桌面宽度都没有横向溢出

构建成功不能替代页面检查。

如果要声明“Codex 实机验证”或提交实机截图，还要按 `docs/native-testing.md` 在对应版本的隔离实例中导入。不要把 Gallery 封面写成实机截图。
