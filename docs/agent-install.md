...
默认的 Full Skin 只作用于 Theme Manager 启动的 ChatGPT 进程。Windows 的“始终应用”控制器已经在准确 ChatGPT Beta `26.715.3651.0` 上完成常驻和清理闭环；它通过当前用户级控制器在以后启动时安全重放，而不是把主题永久写进应用。

Agent 只有在官方 Release 明确包含兼容 Windows 版本时，才可以介绍这个开关。开启前必须获得用户同意，因为控制器会注册当前用户登录启动项，并可能关闭和重开用户选择的准确 ChatGPT 通道一次。未知版本保持原生。macOS 真机常驻尚未验证，Agent 不得在 Mac 上承诺该能力。实现与证据见 [persistent-theme.md](persistent-theme.md)。
