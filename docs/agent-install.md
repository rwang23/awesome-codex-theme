...
Agent 只有在官方 Release 明确包含兼容版本时，才可以介绍这个开关。首次应用必须让用户在应用内确认，因为控制器会注册当前用户登录启动项，并可能关闭和重开用户选择的准确 ChatGPT 通道一次。未知版本保持原生。macOS 真机常驻尚未验证，Agent 不得在 Mac 上承诺该能力。实现与证据见 [persistent-theme.md](persistent-theme.md)。
