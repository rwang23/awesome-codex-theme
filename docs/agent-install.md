...
首个 Beta 只要求 Tauri updater 签名。它能让已安装的 Theme Manager 验证后续更新是否来自同一发布者，但不能替代：
...
当前 alpha 的 Full Skin 只作用于 Theme Manager 启动的 ChatGPT 进程。Agent 不应承诺“安装一次永久生效”。正常关闭 ChatGPT 后，再从系统直接打开会恢复原生界面。

“保持主题常驻”已经完成架构设计，但在 Windows 与 macOS 的控制器、卸载恢复和真实应用回归验证全部通过前，不应被 Agent 当作已发布能力。设计见 [persistent-theme.md](persistent-theme.md)。
