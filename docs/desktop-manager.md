...
启动、退出和监听端口所有者都绑定到准确 Bundle ID 与 executable path。以上属于
源码与测试边界；真机上的 LaunchAgent、Gatekeeper 和 Full Skin 读回仍需单独验收。
