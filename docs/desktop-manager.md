...
The manager uses Tauri 2 with a dependency-free frontend and a Rust security boundary. Rust validates the catalog, downloads hash-bound PNGs, launches the exact Stable or Beta Store package with a loopback-only CDP port, injects one fixed runtime, and records enough session state to restore it. Windows build and real-app apply/restore are verified. Public Windows and macOS releases remain gated on signing; macOS also requires notarization and physical-device readback.
